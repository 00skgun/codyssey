...
## 포트 및 볼륨 설정의 재현성 확보 방식
포트 설정: 실행할 때마다 임의의 포트를 쓰지 않고, 문서에 -p 8080:80 옵션을 명시했습니다. 누가 어느 PC에서 실행하든 항상 localhost:8080으로 똑같이 접속할 수 있도록 규격화한 것입니다.

볼륨 설정: 제 PC의 로컬 절대 경로를 직접 적지 않고, Docker가 관리하는 Named Volume을 생성해서 마운트했습니다. 이렇게 하면 macOS, Linux 등 OS 종류나 폴더 구조에 구애받지 않고 명령어 한 줄로 동일한 데이터 저장 환경을 세팅할 수 있습니다.

## 이미지 vs 컨테이너의 차이 (빌드/실행/변경 관점)
빌드: 이미지는 Dockerfile을 이용해 구워낸 읽기 전용 템플릿입니다.

실행: 컨테이너는 이 이미지를 바탕으로 실제 메모리에 올라가서 돌아가고 있는 프로세스입니다.

변경: 이미지는 한 번 만들어지면 수정할 수 없기 때문에 코드가 바뀌면 이미지를 새로 빌드해야 합니다. 반면 컨테이너는 안에서 파일을 수정하거나 패키지를 깔 수 있지만, 컨테이너를 삭제하면 변경했던 내역들도 다 같이 날아갑니다.
...
터미널에서 lsof -i :8080 명령어를 쳐서 지금 8080 포트를 잡고 있는 프로세스(PID)가 뭔지 찾습니다.

안 끄고 놔둔 이전 테스트용 Docker 컨테이너인지, 아니면 다른 필수 프로그램이 쓰고 있는지 확인합니다.

놔둬도 되는 거면 kill -9 [PID]로 프로세스를 죽이거나 docker rm -f로 예전 컨테이너를 지워서 포트를 비워줍니다. 죽이면 안 되는 필수 프로세스라면 docker run -p 8081:80처럼 내 포트를 딴 걸로 바꿔서 우회 실행합니다.
...
curl: (7) Failed to connect to localhost port 8080 after 0 ms: Couldnt connect to server
